...
## 2025-02-12 - Replace JSON.parse(JSON.stringify) with structuredClone
**Learning:** Found two instances in `app/admin/components/SettingsEditor.tsx` where deep copies of the settings objects were made using `JSON.parse(JSON.stringify)`. The codebase allows swapping out string serialization overhead with native `structuredClone()`, but due to strict typing in `SettingsEditor.tsx` where dynamic string keys are assigned (like `obj[key]`), `structuredClone` must be explicitly cast `as any` to prevent TypeScript index signature errors, as it preserves strict object types unlike the default `any` returned by `JSON.parse`.
**Action:** When migrating deep clones to `structuredClone` on strictly typed objects that subsequently undergo dynamic string-key mutations, always ensure to cast the resulting clone (e.g., `as any`) if the interface does not natively define an index signature.
