...
## 2024-05-30 - [O(1) Memory reduction for bucketing operations]
**Learning:** When bucketing or grouping large datasets (e.g., thousands of map coordinates into geographical clusters), memory-heavy array allocations (`[].push()`) followed by a `reduce` operation can consume significant memory and CPU cycles.
**Action:** Replace array aggregations with running statistical accumulators (e.g., `latSum`, `lngSum`, `count`) directly on the bucket. This reduces bucket memory from O(N) to O(1) and eliminates expensive post-processing loops.
