...
## 2025-02-18 - [Strict Content-Type Validation in API Routes Excluding CSP]
**Vulnerability:** Endpoints that proxy user-controlled content (like the image proxy API) but are excluded from the CSP matcher (e.g. `/api/*`) are vulnerable to Stored XSS if they serve dangerous MIME types like `image/svg+xml` or `text/html`.
**Learning:** CSP rules defined in middleware do not apply to routes excluded by the matcher. Proxied content can bypass security if the `Content-Type` header is directly propagated without strict validation.
**Prevention:** Always enforce strict whitelist validation on `Content-Type` headers for proxied content. Convert to lowercase, apply specific positive mappings, block dangerous types (like `svg` and `xml`), and fall back to `application/octet-stream` for anything not strictly matching `image/` or `video/`.
