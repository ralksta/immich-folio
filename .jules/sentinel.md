...
## 2024-05-18 - XSS in Next.js API Routes despite global CSP
**Vulnerability:** API routes serving user-controlled content were vulnerable to Stored XSS because the middleware CSP explicitly excluded `/api/*` routes.
**Learning:** Next.js middleware matchers that exclude `api` paths (a common practice to prevent API request issues) leave API endpoints serving proxy content completely unprotected by CSP.
**Prevention:** Always implement strict Content-Type validation and sanitization on proxy API endpoints that stream external content, enforcing safe types (like `image/jpeg` or fallback to `application/octet-stream`) and blocking execution types like SVG or XML.
