...
## 2024-05-19 - Content-Type Sanitization Stored XSS Prevention
**Vulnerability:** The image proxy (`app/api/image/[id]/route.ts`) trusted the Content-Type returned by Immich. Because the route falls outside the CSP middleware matcher, serving files like `Image/SVG+XML` with embedded scripts would result in Stored XSS when opened directly.
**Learning:** Next.js dynamic API routes are excluded from standard CSP middleware. Therefore, any endpoint serving user-provided files must strictly sanitize and validate the `Content-Type` header natively (enforcing `image/` or `video/` and blocking `svg`/`xml`) to prevent script execution.
**Prevention:** Convert headers to lowercase to stop case-sensitive bypasses, apply positive remaps first (e.g., `octet-stream` to `image/jpeg`), and finally fall back unsafe types to `application/octet-stream` to force downloads instead of inline execution.
