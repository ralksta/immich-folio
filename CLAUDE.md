...
A fourth gate covers the **whole site**: `sitePassword` in `settings.yaml` (or `SITE_PASSWORD`, which overrides it), cookie `lb_site_auth`, type `'site'` through the same `authenticate()`/`isAuthenticated()` path as the rest. The name breaks the `lb_auth_<slug>` pattern on purpose, and the signed payload uses `__site__` rather than `site`: a subpage may be slugged `site`, and sharing either the cookie name or the HMAC key with it would let the two gates overwrite — or with equal passwords, unlock — each other.
