/**
 * Journal & Storytelling Service.
 * Provides parsing, serialization, and atomic file storage for Journal entries (content/journal/*.md).
 */
...
export function serializeJournalMarkdown(journal: ParsedJournal): string {
  ...
  if (fmKeys.length > 0 && fmKeys.some((k) => journal.frontmatter[k] !== undefined && journal.frontmatter[k] !== '')) {
    lines.push('---');
...
// ── File System & Storage Service (Server-side only) ─────────────────
