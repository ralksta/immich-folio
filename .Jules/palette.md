...
## 2025-06-25 - ARIA Labels for Text Icon Buttons
**Learning:** Using simple text symbols like "×" for close or clear buttons without ARIA labels leaves screen reader users without adequate context, as the symbol may be read out as a literal letter or ignored.
**Action:** Always provide an explicit `aria-label` for buttons that rely on text symbols or icons for their meaning, and hide the visual symbol from screen readers using `aria-hidden="true"`.
