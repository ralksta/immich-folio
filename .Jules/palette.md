...
## 2024-05-24 - Text-Based Icon Buttons Accessibility
**Learning:** Text-based icon buttons (e.g., using "×" or "✕" for close/clear actions) are read literally by screen readers (e.g., "times" or "multiplication X") if not properly labeled, leading to a confusing user experience.
**Action:** When fixing accessibility for text-based icon buttons, wrap the visual text character in `<span aria-hidden="true">` and add a descriptive `aria-label` to the parent `<button>` to ensure correct screen reader announcements.
