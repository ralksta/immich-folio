...
## 2024-06-16 - Make text-based icon buttons accessible
**Learning:** Text-based icon buttons (like "×") need explicit screen reader support because the character might be announced improperly or confusingly by screen readers.
**Action:** Always wrap visual text characters used as icons in `<span aria-hidden="true">` and add an explicit `aria-label` to the parent `<button>`.
